...
const barWidth = (ms: number) =>
  `${Math.min(Math.max((ms / 50) * 100, 2), 100)}%`;
      ...
      {record ? (
        <>
          ...
          <div className={styles.kv}>
            ...
            <b className={styles.mono}>{record.operationId}</b>
          </div>
          <div className={styles.kv}>
            <span>触发 → commit</span>
            <b>
              {record.triggerToCommitMs >= 0
                ? `${record.triggerToCommitMs} ms`
                : '—'}
            </b>
          </div>
          ...
          <div className={styles.bar}>
            <div
              className={styles.barFill}
              style={{ width: barWidth(record.renderMs) }}
            />
          </div>
          {record.frames ? (
            <>
              <div className={styles.kv}>
                <span>动画帧</span>
                <b>
                  {record.frames.frameCount} 帧 · avg {record.frames.avgMs} ms ·
                  max {record.frames.maxMs} ms
                </b>
              </div>
              <div className={styles.bar}>
                <div
              ...
              <div className={styles.kv}>
                ...
                <b
                  className={
                    record.frames.jankCount > 0 ? styles.bad : styles.good
                  }
                >
                  {record.frames.jankCount} 次
                </b>
              </div>
              <div className={styles.kv}>
                <span>采样窗口</span>
                <b>{record.frames.windowMs ?? '—'} ms</b>
              </div>
              <p className={styles.hint}>
                60Hz 每帧预算约 16.67ms；&gt;32ms
                表示至少错过一帧。它是帧间隔告警，不等于精确丢失画面数量。
                结合本次 operationId 和动画/rAF 事件，再到 DevTools Performance
                的 Main / Call Tree 查看源码。
              </p>
            </>
          ) : (
            <div className={styles.kv}>
          ...
          <p className={styles.hint}>
            定位顺序：先记住 operationId → 在“事件循环”面板看 Long Task
            与阶段重叠 → 在 DevTools Performance 中按同一时间窗查看 Main / Call
            Tree。GSAP ticker 可能受 lagSmoothing 钳制，掉帧数量不是唯一根因。
          </p>
        </>
      ) : (
